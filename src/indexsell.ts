...
const swap = async () => {
  ...
  const tokenAAmount = 100; // e.g. 0.01 SOL -> B_TOKEN

  const baseMint = 'So11111111111111111111111111111111111111112'; // e.g. SOLANA mint address
  const quoteMint = 'DVuaDuQdPZ6H49inC2Xoyx7BpLAAJTPPChSfHuGpy8X4'; // e.g. PLANK mint address

  const rpcUrl = '<YOUR RPC>';
  const walletPrivateKey = '<PRIVATE KEY>';
  ...
  if (!poolInfo) {
    poolInfo = await raydiumSwap.findRaydiumPoolInfo(baseMint, quoteMint);
  }
